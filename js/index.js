  ...
  if (changeRequest.readyState === 4) {
    if (changeRequest.status === 200 || changeRequest.status == 0) {
      ...
      JSON.parse(changeRequest.responseText).forEach(function(change) {
        ...
        html += "<div id=" + newName + " class=\"card\"><div class=\"card-content\"><span class=\"card-title\">" + change.name + "</span><div class=\"right reldate\">[" + releaseType + "] " + change.published_at + "</div><ul>" +  change.body + "</div></div>";
